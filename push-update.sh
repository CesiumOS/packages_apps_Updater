...
updates_dir=/data/cesiumos_updates

if [ ! -f "$1" ]; then
   echo "Usage: $0 ZIP [UNVERIFIED]"
   echo "Push ZIP to $updates_dir and add it to Updater"
   echo
   echo "The name of ZIP is assumed to have $(CESIUM_MOD_VERSION) as format"
...
adb shell "killall org.cesium.updater 2>/dev/null"
adb shell "sqlite3 /data/data/org.cesium.updater/databases/updates.db" \
